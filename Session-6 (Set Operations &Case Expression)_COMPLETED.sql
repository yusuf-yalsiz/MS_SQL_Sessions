...
/* UNION Operatörü:
UNION operatörü, iki veya daha fazla SELECT ifadesinin sonuçlarını birleştirir ve tekrar eden satırları kaldırır.
İlk SELECT ifadesinin sonuçlarına ikinci SELECT ifadesinin sonuçları eklenir.
UNION operatörü kullanıldığında, her iki sorgunun da aynı sütun yapısına sahip olması gerekir.

  INTERSECT Operatörü:
INTERSECT operatörü, iki SELECT ifadesinin kesişimini döndürür, yani her iki SELECT ifadesinin sonuçlarında bulunan ortak satırları getirir.
Her iki SELECT ifadesinin sonuçlarında ortak sütun yapısına sahip olması gerekir.
INTERSECT operatörü, tekrar eden satırları otomatik olarak kaldırır.*/
-----------------------------------------------------------
SELECT product_name
FROM sale.customer c
INNER JOIN sale.orders o ON c.customer_id = o.customer_id
...
/*İlk İki Sorgu (UNION ile birleştirilmiş):
İlk iki sorgu, 'Charlotte' ve 'Aurora' şehirlerindeki müşterilerin sipariş ettiği ürünleri birleştirir.
Bu sorgular, iki ayrı sorgu sonucunu birleştirerek, sonuç kümesinde her bir ürünün yalnızca bir kez bulunmasını sağlar.
Müşterilerin şehirlerine göre ayrılmış olan bu sorgular, sonuç kümesindeki her ürünün yalnızca bir kez görünmesini garanti eder.
Üçüncü Sorgu (IN ile filtrelenmiş):
Üçüncü sorgu, 'Charlotte' veya 'Aurora' şehirlerindeki müşterilerin sipariş ettiği ürünleri getirir.
Bu sorgu, IN operatörünü kullanarak tek bir sorgu içinde birden fazla şehri filtreler.
Ancak, bu sorgu sonucunda, her ürünün birden fazla kez görünme olasılığı vardır; 
çünkü sorgu, her iki şehirdeki müşterilerin siparişlerini birleştirmez, sadece bu şehirlerdeki müşterilerin toplam siparişlerini getirir. */
...
/* İlk iki sorgu, 'Charlotte' ve 'Aurora' şehirlerindeki müşterilerin sipariş ettiği ürünleri birleştirirken,
üçüncü sorgu bu şehirlerdeki müşterilerin toplam siparişlerini getirir ve sonuçları temizler.
Ancak sonuçlar aynı olmalıdır, çünkü UNION operatörü zaten tekrar eden satırları kaldırır.
Eğer herhangi bir fark varsa, bu, UNION ile birleştirilen sorgulardan kaynaklanır. */
...
INNER JOIN product.product p ON oi.product_id=p.product_id
WHERE city IN ('Aurora', 'Charlotte');
---Aslinda bununla ayni sey. Yani tekrarlari onlememis oluyoruz
...
FROM product.product A, product.brand B
WHERE	A.brand_id = B.brand_id
	AND	model_year = 2018 AND model_year = 2020

/* Bu sorguda, model yılı hem 2018 hem de 2020 olan ürünlerin marka bilgilerini getirmeye çalışıyorsunuz.
Ancak, aynı anda model yılının hem 2018 hem de 2020 olması mümkün değildir. 
Bu nedenle, bu sorgu hiçbir sonuç döndürmeyecektir. */
...
- Oluşturulacak yeni sütunu, END keywordunden sonra AS keywordü ile adlandırıyoruz.
...
SELECT	CASE
			...
			WHEN email IS NOT NULL THEN 'Other' 
			ELSE NULL 
		END Mail, COUNT (customer_id) AS Counts
FROM	sale.customer
GROUP BY
	CASE
		WHEN email LIKE '%@gmail.%' THEN 'Gmail' 
