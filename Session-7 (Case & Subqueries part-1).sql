...
FROM	sale.staff
---
SELECT	first_name, last_name, A.store_id, store_name
...
/* Write a query that gives the first and last names of customers
   who have ordered products from the computer accessories, speakers, and mp4 player
   categories in the same order.
   Ayný sipariþte hem mp4 player,hem Computer Accessories 
   hem de Speakers kategorilerinde ürün sipariþ veren müþterileri bulunuz.*/
...
--diagramda bagli degil ama city ortak sutun

--//////

--Write a query that returns the list of products that are 
--more expensive than the product named 'Pro-Series 49-Class Full HD Outdoor LED TV (Silver)'
...
WHERE
		X.order_id IN (     SELECT	TOP (10) order_id
							FROM	sale.customer A 
									INNER JOIN
									sale.orders B
									ON A.customer_id = B.customer_id
