...
SELECT	order_id, order_date, shipped_date,
	  CASE  WHEN shipped_date IS NULL THEN 'Not Shipped'
			...
			WHEN DATEDIFF(month, order_date, shipped_date) <> 0 THEN 'Problem'
		END AS order_label
FROM	sale.orders
...
SELECT	order_id, order_date, shipped_date,
	  CASE shipped_date WHEN  NULL THEN 'Not Shipped'
			            WHEN  order_date THEN 'Fast'
		END AS order_label
FROM	sale.orders
...
AND		list_price > (  SELECT	TOP 1 list_price
...
/*"EXISTS" ifadesi, bir alt sorgunun sonucunda en az bir satırın döndüğünübelirlemek için
kullanılır. Genellikle bir ana sorgunun içinde bir alt sorguyla birlikte kullanılır. */
...
---kontrol fonksiyonu. subquery herhangi bir deger döndürmedigi için sonuç bos küme.
SELECT *
FROM	sale.customer
WHERE	EXISTS (
				SELECT *
...
WHERE	EXISTS ( SELECT	1
				 FROM	sale.orders AS B
				 WHERE	order_date > '2020-01-01'
				 AND	A.customer_id = B.customer_id) --268 deger(ortak olanlar)
...
ON A.customer_id = B.customer_id AND B.order_date > '2020-01-01';

/*SELECT A.*
FROM sale.customer AS A
INNER JOIN sale.orders AS B 
ON A.customer_id = B.customer_id AND B.order_date > '2020-01-01';
Buradaki degerler DISTINCT degil unutma ondan dolayi satir sayisi farkli*/
...
WHERE	EXISTS (
				SELECT	1
				FROM	sale.orders AS B
				WHERE	order_date > '2020-01-01') --2000 deger
...
--- eger subquery sonuç döndürmüþse not exists için koþul saðlanmamýþ demektir ve outor query' den sonuç dönmez.
...
/*Null deger dedikten sonra distinct dememize gerek yok. Cunku siparis vermemis kisiler 
order dan dolayi tekrarlamayack cunku zaten order lari yok..
Yani left join de sadece diger tabloda mevcut id ler de tekrarlama olacak*/

--all customers in the sale.customer table who do not have any orders after January 1st, 2020.

SELECT *
FROM	sale.customer AS A, sale.orders AS B
WHERE	A.customer_id = B.customer_id
--Bu artik inner join tablosu yani who had orders!
...
WHERE	order_date > '2020-01-01'		
AND		A.customer_id = B.customer_id
--who had orders after January 1st, 2020 tablosu.
-- (1615 - 286)
...
--- it finds customers who had orders before January 1st, 2020.
...
SELECT	DISTINCT [state]
FROM	sale.customer
WHERE	[state] NOT IN (SELECT	 D.[state]
						FROM	product.product AS A
								INNER JOIN
								sale.order_item AS B
								ON A.product_id = B.product_id
...
SELECT	DISTINCT A.product_id, quantity
...
SELECT	order_id, SUM(quantity*list_price) total_list_price
FROM	sale.order_item
GROUP BY order_id ;

SELECT	order_id, 99 AS total_price
FROM	sale.order_item;


SELECT	order_id, (SELECT SUM(quantity*list_price) FROM sale.order_item) AS total_price
FROM	sale.order_item;
--Gruplamadan bu sekilde de coklayabiliyoruz. OVER gibi...

-------------

SELECT  order_id, (SELECT SUM(quantity*list_price) FROM sale.order_item AS A
                             WHERE A.order_id = B.order_id) AS total_price
FROM	sale.order_item AS B;

SELECT	DISTINCT order_id, (SELECT SUM(quantity*list_price) FROM sale.order_item AS A
                             WHERE A.order_id = B.order_id) AS total_price
FROM	sale.order_item AS B;

-- ORDER_ID YE GORE GRUPLAYIP TOPLADI..
...
--ORDINARY CTE's (Common Table Expression)
...
SELECT	X.order_date,  Y.first_name, Y.last_name
...
FROM	sale.orders AS X
		...
		sale.customer AS Y
		ON X.customer_id = Y.customer_id
WHERE	Y.first_name = 'Jerald'
		AND	Y.last_name = 'Berray'
		AND	X.order_date < (SELECT MAX(order_date) 
                            FROM sale.orders 
                            WHERE customer_id = Y.customer_id)
		AND	Y.city = 'Austin';
--Aradaki fark?
--Jerald Berray in sadece son tarih bilgisini kullanmamiz gerekiyrodu ama kendisini kullandik
/*
CREATE VIEW V_T1 AS
SELECT	MAX(order_date) last_order_date
FROM	sale.customer AS A
...
--Esitsizlik oldugu için hatali sonuç verdi.
