...
-- 1-List customers whose have at least 2 consecutive orders are not shipped
with T1 as (
		select customer_id,order_date, shipped_date,
		lag(shipped_date) over (partition by customer_id order by order_date) prev_date,
		lead(shipped_date) over (partition by customer_id order by order_date) next_date
		from sale.orders
		 
		)
select *
from T1
where prev_date is Null and next_date is Null and shipped_date is Null;


-- 2-Write a query that returns the higest daily turnover amount for each week on a yearly basis 
with T1 as (
			select year(order_date) ord_year,
				datepart(ISOWK,order_date) ord_week,
				datepart(DY,order_date) ord_day,
				sum(list_price* quantity * (1-discount)) over (partition by datepart(DY, order_date) order by order_date) sum_day
...
max(sum_day) over (partition by ord_year, ord_week) highest_turnover
from T1
------------
WITH T1 AS(
SELECT	 O.order_id AS Order_id,
		...
		order_date,
		DATEPART(ISOWK,order_date) WEEK,
		SUM(((1-discount)*quantity)*list_price) OVER(PARTITION BY O.order_id ORDER BY order_date) Ord_sum
...
SELECT *,
		SUM(Ord_sum) OVER(PARTITION BY order_date ORDER BY order_date) turnover
FROM T2
) 
SELECT  DISTINCT year,
		week,
		MAX(turnover) OVER(PARTITION BY YEAR(order_date),DATEPART(ISOWK,order_date)  ORDER BY DATEPART(ISOWK,order_date)) Weekly_Max
FROM T3