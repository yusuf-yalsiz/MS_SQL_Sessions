...
ALTER TABLE t_date_time
ADD new_column DATETIME;
...
SELECT GETDATE(),
       DATEDIFF(YEAR, '1991-09-18', GETDATE()),
       DATEDIFF(DAY, '1991-09-18', GETDATE());


SELECT DATENAME(DW, '1991-09-18');
...
-- TOP 10 customer_id, first_name, last_name
...
-- AYIRICI KARAKTER NCHAR(1) VEYA NVARCHAR(1) OLMALI YANI 1 KARAKTER OLMALI
...
SELECT TRIM('Y' FROM 'Yusuf Yalsiy')
...
SELECT STR(1265465455.216, 14, 2) 

SELECT STR(1265465455.216, 13, 3) 
...
SELECT *,  datepart (month, CONVERT(VARCHAR(50), order_date, 106)) as month
FROM	sale.orders
...
SELECT ROUND(123456.5678, 2)   --default 0 aliyor

SELECT ROUND(123456.5678, 2, 1)


SELECT ROUND(123456.5638, 2, 1) --5600


SELECT phone, ISNULL (phone, '+111111111')
FROM	sale.customer

--e?er telefon numaras? NULL ise yerine '+111111111' de?erini döndürecektir. ISNULL(expression, alternative_value)
...
--NULL olmayan ilk ifadeyi döndürür. E?er tüm ifadeler NULL ise, COALESCE NULL döndürür.

SELECT phone, COALESCE (phone, '1')
FROM	sale.customer

--REPLACE fonksiyonu NULL de?erleri do?rudan i?leyemez. Bunun yerine, COALESCE veya ISNULL fonksiyonu ile NULL de?erlerin yerine geçici bir de?er atanabilir
...
----NULLIF iki ifadeyi kar??la?t?r?r. E?er ifadeler e?itse, NULL de?eri döndürür. E?er ifadeler farkl?ysa, ilk ifadeyi döndürür.
...
--yahoo mailine sahip kaç müþteri vardir?
...
SELECT 
    email, 
    SUBSTRING(email, 1, CHARINDEX('@', email)-1) AS kullanici_adi,
    SUBSTRING(email,CHARINDEX('@', email)+1, LEN(email)) AS domain_adi
FROM 
    sale.customer;
...
SELECT 
    ...
    m.first_name AS first_manager_name,
    m1.first_name AS second_manager_name
FROM 
    sale.staff s
LEFT JOIN 
    sale.staff m ON s.manager_id = m.staff_id
LEFT JOIN 
    sale.staff m1 ON m.manager_id = m1.staff_id
ORDER BY second_manager_name;
