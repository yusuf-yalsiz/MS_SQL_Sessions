...
-- SADECE YILLARA GORE GRUPLAMA YOK
...
SELECT Model_Year, total_turnover 
FROM v_category_model_turnover
...
SELECT *
FROM (
		SELECT Model_Year, total_turnover FROM v_category_model_turnover
		) A 
PIVOT
(
SUM(total_turnover)
...
SELECT DATENAME(DW, order_date) AS week_day, COUNT(DATENAME(DW, order_date)) AS [count]
FROM sale.orders
...
SUM([count])
FOR week_day
IN ('Monday', 'Sunday', 'Wednesday', 'Thursday', 'Friday')
) AS PVT
...
FROM 
    (   SELECT 
            ...
            COUNT(DATENAME(DW, order_date)) AS [count]
        FROM 
            sale.orders
        WHERE 
